...
pub struct MemoryManager {
    ...
    buffer: Vec<u8>,
    free_blocks: Vec<Option<MemoryBlock>>, // create a free_block struct
    allocated_blocks: Vec<Option<MemoryBlock>>, // create a allocated_block struct
    next_id: usize,
}
...
impl MemoryManager {
    ...
    pub fn find(&self, id: usize) -> Result<&MemoryBlock, String> {
        ...
        self.blocks
            .get(id) // store id inside of allocated_block and search for matching id
            .and_then(|b| b.as_ref())
        ...
        for (id, block_option) in self.blocks.iter().enumerate() {
            if let Some(block) = block_option {
                println!(
                    ...
                    block.end,
                    block.end - block.start,
                    block.get_data() // call self.get_data(block);
                );
            }
        }
    }
